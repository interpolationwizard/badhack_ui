		...
		virtual void render( ) override {
			...
			if ( m_active ) {
				...
				if ( g_input.is_key_pressed( KEYS_MOUSE1 ) && !m_mouse_held
					...
					int mouse_x_c = std::min< int >( std::max< int >( mouse_x, x ), x + 100 );
					int mouse_y_c = std::min< int >( std::max< int >( mouse_y, y ), y + 100 );

					int delta_y = std::min< int >( std::max< int >( mouse_y_c - y, 0 ), 100 );
					int delta_x = std::min< int >( std::max< int >( mouse_x_c - x, 0 ), 100 );
				...
				if ( is_hue_slider_hovered( ) && g_input.is_key_pressed( KEYS_MOUSE1 ) ) {
					int delta = std::min< int >( std::max< int >( mouse_y - y, 0 ), 100 );
					m_hue = float( delta ) * 0.01f;
				}

				draw_slider_alpha( );
				if ( is_alpha_slider_hovered( ) && g_input.is_key_pressed( KEYS_MOUSE1 ) ) {
					int delta = std::min< int >( std::max< int >( mouse_x - x, 0 ), 121 );
